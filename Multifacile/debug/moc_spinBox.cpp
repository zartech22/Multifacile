/****************************************************************************
** Meta object code from reading C++ file 'spinBox.h'
**
** Created: Sat 18. May 21:16:33 2013
**      by: The Qt Meta Object Compiler version 63 (Qt 4.8.4)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../spinBox.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'spinBox.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 63
#error "This file was generated using the moc from 4.8.4. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_SpinBox[] = {

 // content:
       6,       // revision
       0,       // classname
       0,    0, // classinfo
       3,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       3,       // signalCount

 // signals: signature, parameters, type, tag, flags
       9,    8,    8,    8, 0x05,
      32,   30,    8,    8, 0x05,
      65,   30,    8,    8, 0x05,

       0        // eod
};

static const char qt_meta_stringdata_SpinBox[] = {
    "SpinBox\0\0enterKeyPressed(int)\0,\0"
    "moveEvent(QMouseEvent*,SpinBox*)\0"
    "pressEvent(QMouseEvent*,SpinBox*)\0"
};

void SpinBox::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Q_ASSERT(staticMetaObject.cast(_o));
        SpinBox *_t = static_cast<SpinBox *>(_o);
        switch (_id) {
        case 0: _t->enterKeyPressed((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 1: _t->moveEvent((*reinterpret_cast< QMouseEvent*(*)>(_a[1])),(*reinterpret_cast< SpinBox*(*)>(_a[2]))); break;
        case 2: _t->pressEvent((*reinterpret_cast< QMouseEvent*(*)>(_a[1])),(*reinterpret_cast< SpinBox*(*)>(_a[2]))); break;
        default: ;
        }
    }
}

const QMetaObjectExtraData SpinBox::staticMetaObjectExtraData = {
    0,  qt_static_metacall 
};

const QMetaObject SpinBox::staticMetaObject = {
    { &QLineEdit::staticMetaObject, qt_meta_stringdata_SpinBox,
      qt_meta_data_SpinBox, &staticMetaObjectExtraData }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &SpinBox::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *SpinBox::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *SpinBox::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_SpinBox))
        return static_cast<void*>(const_cast< SpinBox*>(this));
    return QLineEdit::qt_metacast(_clname);
}

int SpinBox::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QLineEdit::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 3)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 3;
    }
    return _id;
}

// SIGNAL 0
void SpinBox::enterKeyPressed(int _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}

// SIGNAL 1
void SpinBox::moveEvent(QMouseEvent * _t1, SpinBox * _t2)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)), const_cast<void*>(reinterpret_cast<const void*>(&_t2)) };
    QMetaObject::activate(this, &staticMetaObject, 1, _a);
}

// SIGNAL 2
void SpinBox::pressEvent(QMouseEvent * _t1, SpinBox * _t2)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)), const_cast<void*>(reinterpret_cast<const void*>(&_t2)) };
    QMetaObject::activate(this, &staticMetaObject, 2, _a);
}
QT_END_MOC_NAMESPACE
